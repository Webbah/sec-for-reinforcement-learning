...
import openmodelica_microgrid_gym
from OMG.env.env_wrapper import FeatureWrapper, FeatureWrapper_pastVals, FeatureWrapper_futureVals, \
    BaseWrapper, FeatureWrapper_I_controller
from OMG.env.rewards import Reward

from OMG.util.recorder import Recorder
...
model_path = 'OMG/trained_agents/'
folder_name = 'OMG/data/new_pkls'
...
def run_OMG_experiment(max_episode_steps=None):
    ...
    data_str = 'OMG/data/R_load_hard_test_case_10_seconds.pkl'
    ...
    net = Network.load('OMG/net/net_vctrl_single_inv.yaml')
    ...
    for max_eps_steps in tqdm(range(len(max_episode_steps_list)), desc='steps', unit='step', leave=False):
        ...
        for ave_run in tqdm(range(num_average), desc='steps', unit='step', leave=False):
            ...
            rand_load_test = RandomLoad(max_episode_steps_list[max_eps_steps], net.ts, gen,
                                        load_curve=pd.read_pickle(data_str))
            ...
            env = gym.make('openmodelica_microgrid_gym:ModelicaEnv_test-v1',
            #env = gym.make('OMG.env:vctrl_single_inv_test-v0',
                           ...
                           net=net,
                           model_path='OMG/net/grid.paper.fmu',
                           history=FullHistory(),
            ...
            pi_df = pd.DataFrame([compare_result])
            pi_df.to_pickle(f'{folder_name}/PI_{max_episode_steps_list[max_eps_steps]}steps')
            ...
            net = Network.load('OMG/net/net_vctrl_single_inv_dq0.yaml')  # is used from vctrl_single_env, not needed here

            for used_model, wrapper_mode, used_number_past_vales in zip(model_name, wrapper, number_past_vals):


                env_test = gym.make('OMG.env:vctrl_single_inv_test-v0',
                ...
                DDPG_df = pd.DataFrame([compare_result])
                DDPG_df.to_pickle(
